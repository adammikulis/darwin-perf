"""High-level Python wrapper functions for darwin-perf.

These build on the low-level C extension functions in ``_native`` to provide
convenient one-call APIs: ``stats``, ``snapshot``, ``GpuMonitor``, etc.
"""
...
def stats(pid: int = 0, interval: float = 0.1) -> dict:
    ...
    When interval > 0, blocks for that duration to compute delta-based
    metrics (per-process CPU%, GPU%, power). When interval <= 0, returns
    instant-only metrics with no sleep (<100us).

    Args:
        pid: Process ID (0 = current process, None = system only).
        interval: Delta sampling interval in seconds (default 0.1s).
            Set to 0 for instant metrics in hot loops.

    Returns:
        Dict with keys:
            Process (when pid is set and interval > 0):
                - proc_cpu_pct, proc_gpu_pct, proc_memory_gb, proc_threads
            Process (when pid is set and interval <= 0):
                - proc_memory_gb, proc_threads
            GPU:
                - gpu_util_pct, gpu_model, gpu_cores, gpu_vram_gb
                - gpu_power_w, gpu_freq_mhz, gpu_throttled (only when interval > 0)
            CPU:
    ...
    if pid is not None:
        ...
        if instant:
            try:
                info = proc_info(pid)
                if info:
                    result["proc_memory_gb"] = round(info.get("real_memory", 0) / 1e9, 1)
                    result["proc_threads"] = info.get("threads", 0)
            except Exception:
                pass
        else:
            try:
    ...
    # --- Temperatures (skip in instant mode — SMC reads are ~1ms each) ---
    if not instant:
        try:
            t = temperatures()
